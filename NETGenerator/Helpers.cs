...
namespace PascalABCCompiler.NETGenerator {
	...
	public class Helper {
        ...
        public TypeInfo GetTypeReference(ITypeNode type)
		{
            ...
            if (type is ICompiledTypeNode ictn) {
                Mono.Cecil.IGenericParameterProvider context = null;

                if (ictn.is_generic_parameter)
                {
                    if (ictn.generic_function_container != null)
                    {
                        var paramOwner = (ICompiledMethodNode)ictn.generic_function_container;
                        context = module.ImportReference(paramOwner.method_info);
                    }
                    else
                    {
                        var paramOwner = (ICompiledTypeNode)ictn.generic_type_container;
                        context = module.ImportReference(paramOwner.compiled_type);
                    }
                }

                ti = new TypeInfo(module.ImportReference(ictn.compiled_type, context));
                defs[type] = ti;
                return ti;
            }
