...
namespace PascalABCCompiler.NETGenerator
{
    ...
    public class CompilerOptions
    {
        ...
        public Mono.Cecil.TypeReference RtlPABCSystemType;
    ...
    public class ILConverter : AbstractVisitor
    {
                ...
                if (ElementValues[0] is IRecordConstantNode)
                {
                    ...
                    for (int i = 0; i < ElementValues.Length; i++)
                    {
                        ...
                        if (ti != null && ti.tp.IsValueType && !TypeFactory.IsStandType(ti.tp) && 
                            (ti.tp.IsGenericInstance || !ti.tp.Resolve().IsEnum)) // инстанцированные генерик типы бросают исключение при вызове .IsEnum
                                il.Emit(OpCodes.Ldelema, ti.tp);
                        var ilb = this.il;
                        this.il = il;
                        ElementValues[i].visit(this);
        ...
        private void AddSpecialMembersToArray(SemanticTree.ICommonClassFieldNode value, Mono.Cecil.FieldAttributes fattr)
        {
            ...
            cb_il.Emit(OpCodes.Call, mb.ImportReference(TypeFactory.ObjectType.GetConstructor(Type.EmptyTypes)));
        ...
        private Mono.Cecil.FieldAttributes ConvertFALToFieldAttributes(field_access_level fal)
        {
            switch (fal)
            {
        ...
        private Mono.Cecil.FieldAttributes AddPSToFieldAttributes(polymorphic_state ps, Mono.Cecil.FieldAttributes fa)
        {
            switch (ps)
            {
        ...
        private MethInfo ConvertCtorWithNested(SemanticTree.ICommonMethodNode meth, Mono.Cecil.MethodDefinition methodb)
        {
            ...
            //определяем метод
            Mono.Cecil.MethodDefinition methb = new Mono.Cecil.MethodDefinition("cnstr$" + uid++, Mono.Cecil.MethodAttributes.Public | Mono.Cecil.MethodAttributes.Static, methodb.DeclaringType);
            foreach (var paramType in param_types)
                methb.Parameters.Add(new Mono.Cecil.ParameterDefinition(paramType));
            tb.Methods.Add(methb);
            MethInfo mi = null;
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                pb.Name = parameters[i].name;
                if (parameters[i].is_params)
                    pb.CustomAttributes.Add(new CustomAttributeBuilder(TypeFactory.ParamArrayAttributeConstructor, new byte[] { 0x1, 0x0, 0x0, 0x0 }).Build());
                Mono.Cecil.FieldDefinition fb = null;
                if (parameters[i].parameter_type == parameter_type.value)
                {
                    fb = new Mono.Cecil.FieldDefinition(parameters[i].name, Mono.Cecil.FieldAttributes.Public, param_types[i + num]);
        ...
        private MethInfo ConvertMethodWithNested(SemanticTree.ICommonMethodNode meth, Mono.Cecil.MethodDefinition methodb)
        {
            ...
            //определяем метод
            Mono.Cecil.MethodDefinition methb = new Mono.Cecil.MethodDefinition(methodb.Name, Mono.Cecil.MethodAttributes.Public | Mono.Cecil.MethodAttributes.Static, methodb.ReturnType);
            tb.Methods.Add(methb);
            MethInfo mi = null;
            //добавляем его фиктивно (т.е. не заносим в таблицы Helper-а) дабы остальные вызывали метод-заглушку
            mi = helper.AddFictiveMethod(meth, methb);
            ...
            Mono.Cecil.FieldDefinition[] fba = new Mono.Cecil.FieldDefinition[parameters.Length];
            //явно определяем this
            pb = new Mono.Cecil.ParameterDefinition("$obj$", Mono.Cecil.ParameterAttributes.None, param_types[0]);
            methb.Parameters.Add(pb);
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                pb = new Mono.Cecil.ParameterDefinition(parameters[i].name, Mono.Cecil.ParameterAttributes.None, param_types[i + num]);
                methb.Parameters.Add(pb);
                Mono.Cecil.FieldDefinition fb = null;
                if (parameters[i].parameter_type == parameter_type.value)
                {
        ...
        private void ConvertNestedInMethodFunctionHeader(ICommonNestedInFunctionFunctionNode func, Mono.Cecil.TypeReference decl_type)
        {
            ...
            methb.Parameters.Add(
                new Mono.Cecil.ParameterDefinition("$obj$", Mono.Cecil.ParameterAttributes.None, param_types[0])
            );
			methb.Parameters.Add(
				new Mono.Cecil.ParameterDefinition("$up$", Mono.Cecil.ParameterAttributes.None, param_types[1])
			);
            num = 2;
            IParameterNode[] parameters = func.parameters;
            //
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (parameters[i].is_params)
                {
                    var customAttr = new Mono.Cecil.CustomAttribute(TypeFactory.ParamArrayAttributeConstructor, new byte[] { 0x1, 0x0, 0x0, 0x0 });
        ...
        private void ConvertConstructorHeader(SemanticTree.ICommonMethodNode value)
        {
            ...
            if (value.polymorphic_state == polymorphic_state.ps_static)
            {
                cnstr = new Mono.Cecil.MethodDefinition(".cctor", Mono.Cecil.MethodAttributes.Public | Mono.Cecil.MethodAttributes.Static | Mono.Cecil.MethodAttributes.SpecialName | Mono.Cecil.MethodAttributes.RTSpecialName, mb.TypeSystem.Void);
            ...
            else
            {
                ...
                cnstr = new Mono.Cecil.MethodDefinition(".ctor", attrs, mb.TypeSystem.Void);
                cnstr.HasThis = true;
                cur_type.Methods.Add(cnstr);
            }
        ...
        {
            return (cur_ti.is_arr && ret_type.FullName != TypeFactory.VoidType.FullName && ret_type.IsValueType && !TypeFactory.IsStandType(ret_type) && !ret_type.Resolve().IsEnum);
        }
        ...
        private void ConvertExternalMethod(SemanticTree.ICommonMethodNode meth)
        {
            ...
            if (statements[0] is IExternalStatementNode)
            {
                ...
                cur_type.Methods.Add(methb);
                methb.PInvokeInfo = new Mono.Cecil.PInvokeInfo(Mono.Cecil.PInvokeAttributes.CallConvWinapi, esn.name, new Mono.Cecil.ModuleReference(module_name));
        ...
        private void ConvertMethodParameters(ICommonMethodNode value, Mono.Cecil.MethodDefinition methb)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (parameters[i].is_params)
                    pb.CustomAttributes.Add(
                        new Mono.Cecil.CustomAttribute(TypeFactory.ParamArrayAttributeConstructor, new byte[] { 0x1, 0x0, 0x0, 0x0 })
                    );
            }
        }
        ...
        public override void visit(SemanticTree.ICompiledStaticMethodCallNode value)
        {
            ...
            is_dot_expr = false;
            Mono.Cecil.ParameterReference[] pinfs = value.static_method.method_info.Parameters.ToArray();
            //кладем параметры
            Mono.Cecil.MethodReference mi = value.static_method.method_info;
            ...
            if (mi.DeclaringType.FullName == TypeFactory.ArrayType.FullName && mi.Name == "Resize" && helper.GetTypeReference(value.template_parametres[0]).tp.IsPointer)
            {
                ...
                real_parameters[0].visit(this);
                il.Emit(OpCodes.Callvirt, TypeFactory.ArrayType.Resolve().GetMethods().First(item => item.Name == "get_Length"));
                real_parameters[1].visit(this);
            ...
            for (int i = 0; i < real_parameters.Length; i++)
            {
                ...
                if (ctn2 != null && !(real_parameters[i] is SemanticTree.INullConstantNode) && (ctn3.is_value_type || ctn3.is_generic_parameter) && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.IsInterface))
                    il.Emit(OpCodes.Box, helper.GetTypeReference(ctn3).tp);
                else if (ctn2 != null && !(real_parameters[i] is SemanticTree.INullConstantNode) && ctn4 != null && (ctn4.is_value_type || ctn4.is_generic_parameter) && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.IsInterface))
        ...
        private bool has_debug_conditional_attr(Mono.Cecil.MethodReference mi)
        {
            ...
            var attrArg = (string)attr.Properties.First(item => item.Name == "ConditionString").Argument.Value;
        ...
        public override void visit(SemanticTree.ICompiledMethodCallNode value)
        {
            ...
            Mono.Cecil.MethodReference mi = value.compiled_method.method_info;
        ...
        private bool GenerateStandardFuncCall(ICommonNamespaceFunctionCallNode value, Mono.Cecil.Cil.ILProcessor il)
        {
            ...
            switch (value.namespace_function.SpecialFunctionKind)
            {
                case SpecialFunctionKind.NewArray:
                    ...
                    if (ti.tp.IsValueType && ti.init_meth != null || ti.is_arr || ti.is_set || ti.is_typed_file || ti.is_text_file || ti.tp.FullName == TypeFactory.StringType.FullName)
                    {
        ...
        private void FixPointer()
        {
            ...
            {
                fix_pointer_meth = comp_opt.RtlPABCSystemType
                    .Resolve()
                    .GetMethods()
        ...
        private void EmitArguments(IParameterNode[] parameters, IExpressionNode[] real_parameters)
        {
        	...
        	for (int i = 0; i < real_parameters.Length; i++)
            {
                ...
                TypeInfo ti = null;
                if (parameters[i].type is ICompiledTypeNode && (parameters[i].type as ICompiledTypeNode).compiled_type.FullName == mb.TypeSystem.Char.FullName && parameters[i].parameter_type == parameter_type.var
                    && real_parameters[i] is ISimpleArrayIndexingNode && helper.GetTypeReference((real_parameters[i] as ISimpleArrayIndexingNode).array.type).tp.FullName == TypeFactory.StringType.FullName)
                {
                    copy_string = true;
                }
                //(ssyy) moved up
                ...
                //Дополнительная проверка введена именно для этого.
                bool box_awaited =
                    (ctn2 != null && (ctn2.compiled_type == TypeFactory.ObjectType || ctn2.compiled_type.FullName == TypeFactory.EnumType.FullName) || tn2.IsInterface) && !(real_parameters[i] is SemanticTree.INullConstantNode) 
                	&& (ctn3.is_value_type || ctn3.is_generic_parameter);
                if (!box_awaited && (ctn2 != null && ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || tn2.IsInterface) && !(real_parameters[i] is SemanticTree.INullConstantNode) 
        ...
        {
            if ((from.type.is_value_type || from.type.is_generic_parameter) && !(from is SemanticTree.INullConstantNode) && (LocalType.FullName == TypeFactory.ObjectType.FullName || TypeIsInterface(LocalType) || LocalType.FullName == TypeFactory.EnumType.FullName))
            {
                il.Emit(OpCodes.Box, helper.GetTypeReference(from.type).tp);//упаковка
                return true;
            }
            if (from.conversion_type != null && from.conversion_type.is_value_type && !(from is SemanticTree.INullConstantNode) && (LocalType.FullName == TypeFactory.ObjectType.FullName || TypeIsInterface(LocalType)))
            {
        ...
        private void BoxAssignToParameter(IExpressionNode to, IExpressionNode from)
        {
            ...
            if ((from.type.is_value_type || from.type.is_generic_parameter) && ctn2 != null && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.IsInterface))
            {
                il.Emit(OpCodes.Box, helper.GetTypeReference(from.type).tp);
            }
            else if (from.conversion_type != null && (from.type.is_value_type || from.type.is_generic_parameter) && ctn2 != null && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.IsInterface))
            {
            	il.Emit(OpCodes.Box, helper.GetTypeReference(from.conversion_type).tp);
            }
            CheckArrayAssign(to, from, il);
        ...
        private void AssignToCompiledField(IExpressionNode to, IExpressionNode from)
        {
            ...
            Mono.Cecil.FieldReference fi = value.field.compiled_field;
            is_dot_expr = true;
            value.obj.visit(this);
            is_dot_expr = false;
            from.visit(this);
        ...
        private void AssignToStaticCompiledField(IExpressionNode to, IExpressionNode from)
        {
            ...
            Mono.Cecil.FieldReference fi = value.static_field.compiled_field;
            from.visit(this);
            EmitBox(from, fi.FieldType);
            il.Emit(OpCodes.Stsfld, fi);
        }
        ...
        private void AssignToSimpleArrayNode(IExpressionNode to, IExpressionNode from)
        {
            ...
            else
            {
                ...
                {
                    get_meth = ti.tp.Resolve().GetMethods().First(item => item.Name == "Get");
                    addr_meth = ti.tp.Resolve().GetMethods().First(item => item.Name == "Address");
                    set_meth = ti.tp.Resolve().GetMethods().First(item => item.Name == "Set");
                }
                else
                {
                    List<Mono.Cecil.TypeReference> lst = new List<Mono.Cecil.TypeReference>();
            ...
            else if (from.conversion_type != null && (from.conversion_type.is_value_type || from.conversion_type.is_generic_parameter) && ctn2 != null && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.IsInterface))
            {
                il.Emit(OpCodes.Box, helper.GetTypeReference(from.conversion_type).tp);
            }
        ...
        private void AssignToDereferenceNode(IExpressionNode to, IExpressionNode from)
        {
            ...
            if ((from.type.is_value_type || from.type.is_generic_parameter) && 
                ctn2 != null && 
                (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || 
                (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.compiled_type.Resolve().IsInterface))
               )
            {
                il.Emit(OpCodes.Box, helper.GetTypeReference(from.type).tp);
            }
            else if (from.conversion_type != null && (from.conversion_type.is_value_type || from.conversion_type.is_generic_parameter) && ctn2 != null && (ctn2.compiled_type.FullName == TypeFactory.ObjectType.FullName || ctn2.compiled_type.Resolve().IsInterface))
            {
                il.Emit(OpCodes.Box, helper.GetTypeReference(from.conversion_type).tp);
            }
            CheckArrayAssign(to, from, il);
        ...
        public override void visit(SemanticTree.IBasicFunctionCallNode value)
        {
...

            {
                    ...
                    (real_parameters[1].type is ICompiledTypeNode && !NetHelper.NetHelper.IsStandType((real_parameters[1].type as ICompiledTypeNode).compiled_type) || real_parameters[1].type is ICompiledGenericTypeInstance) && !real_parameters[1].type.is_nullable_type)
                {
                    ...
                    Mono.Cecil.TypeReference t1 = ctn1.compiled_type;
					Mono.Cecil.TypeReference t2 = ctn2.compiled_type;
                    ...
                    Mono.Cecil.MethodReference mi = null;
                    var eq_members = ctn1.compiled_type.Resolve()
                        .GetMethods()
                ...
                if (real_parameters.Length > 1)
                {
                    ...
                    else if (real_parameters[0].type.is_nullable_type && real_parameters[1].type.is_nullable_type)
                    {
                        ...
                        {
                            var ct = (Mono.Cecil.GenericInstanceType)(real_parameters[0].type as ICompiledTypeNode).compiled_type;
        ...
        public void ConvertArrayWrapperType(SemanticTree.ICommonTypeNode value)
        {
            ...
            Mono.Cecil.MethodDefinition mb = new Mono.Cecil.MethodDefinition("$Init$", Mono.Cecil.MethodAttributes.Private, this.mb.TypeSystem.Void);
            tb.Methods.Add(mb);
            ti.init_meth = mb;
        ...
        public override void visit(SemanticTree.ISimpleArrayIndexingNode value)
        {
            ...
            bool string_getter = temp_is_addr && ti.tp.FullName == TypeFactory.StringType.FullName;
            Mono.Cecil.Cil.VariableDefinition pin_lb = null;
            var indices = value.indices;
            if (string_getter)
            {
                pin_lb = new Mono.Cecil.Cil.VariableDefinition(TypeFactory.StringType.MakePinnedType());
        ...
        public override void visit(SemanticTree.ICompiledConstructorCall value)
        {
            ...
            IExpressionNode[] real_parameters = value.real_parameters;
            Mono.Cecil.TypeReference cons_type11 = value.constructor.comprehensive_type.compiled_type;
            ...
            if (tmp_dot == true)
            {
                ...
                {
                    Mono.Cecil.Cil.VariableDefinition lb = new Mono.Cecil.Cil.VariableDefinition(value.constructor.constructor_info.DeclaringType);
                    il.Body.Variables.Add(lb);
                    il.Emit(OpCodes.Stloc, lb);
                    il.Emit(OpCodes.Ldloca, lb);
                }
        ...
        {
            if (value.field.compiled_field.Resolve().IsLiteral == false)
            {
                value.obj.visit(this);
                il.Emit(OpCodes.Ldflda, value.field.compiled_field);
            }
        ...
        {
            if (value.static_field.compiled_field.Resolve().IsLiteral == false)
            {
                il.Emit(OpCodes.Ldsflda, value.static_field.compiled_field);
            }
        }
        ...
        private void PushRuntimeSize(Mono.Cecil.TypeReference t)
        {
            ...
            else
            {
                ...
                prms[0] = typeof(Type);
                il.Emit(OpCodes.Call, typ.GetMethod("SizeOf", prms));
            }
            return;
        }
        ...
        {
            Mono.Cecil.FieldDefinition fb = null;
            if (value.type is ICompiledTypeNode && (value.type as ICompiledTypeNode).compiled_type.Resolve().IsEnum)
            {
        ...
        public override void visit(IForeachNode value)
        {
            ...
            if (is_generic)
            {
                var genericMethod = TypeFactory.IEnumerableGenericType.Resolve()
                    .GetMethods()
                    .Single(item => item.Name == "GetEnumerator");

                var instancedInterf = TypeFactory.IEnumerableGenericType.MakeGenericInstanceType(elementType);
            ...
            {
                enumer_mi = TypeFactory.IEnumerableType.Resolve()
                    .GetMethods()
                    .Single(item => item.Name == "GetEnumerator");
            ...
            {
                get_current_meth = get_current_meth.AsMemberOf( (Mono.Cecil.GenericInstanceType)enumer_mi.ReturnType );
            }
            ...
            if (vi.kind == VarKind.vkLocal)
            {
                ...
                if (vi.lb.VariableType.FullName == TypeFactory.ObjectType.FullName && get_current_meth.ReturnType.IsValueType)
                    il.Emit(OpCodes.Box, get_current_meth.ReturnType);
                else if (vi.lb.VariableType.FullName == TypeFactory.StringType.FullName && get_current_meth.ReturnType.FullName == mb.TypeSystem.Char.FullName)
            ...
            else if (vi.kind == VarKind.vkGlobal)
            {
                ...
                if (vi.fb.FieldType.FullName == TypeFactory.ObjectType.FullName && get_current_meth.ReturnType.IsValueType)
                    il.Emit(OpCodes.Box, get_current_meth.ReturnType);
                else if (vi.fb.FieldType.FullName == TypeFactory.StringType.FullName && get_current_meth.ReturnType.FullName == mb.TypeSystem.Char.FullName)
                    il.Emit(OpCodes.Call, TypeFactory.CharToString);
                il.Emit(OpCodes.Stsfld, vi.fb);
            }
            else
        ...
        public override void visit(ILockStatement value)
        {
            ...
            Mono.Cecil.Cil.VariableDefinition lb = new Mono.Cecil.Cil.VariableDefinition(TypeFactory.ObjectType);
            il.Body.Variables.Add(lb);
