...
namespace PascalABCCompiler.NETGenerator
{
    ...
    public class ILConverter : AbstractVisitor
    {
        ...
        {
            var body = ilg.Body;
            var instr = body.Instructions.LastOrDefault();
        ...
        private void ConvertCompiledGenericInstanceTypeMembers(ICompiledGenericTypeInstance value)
        {
            ...
            foreach (IDefinitionNode dn in value.used_members.Keys)
            {
                ...
                if (iccn != null)
                {
                    var ci = mb.ImportReference(iccn.constructor_info, t.ElementType).AsMemberOf(t);
                ...
                if (icmn != null)
                {
                    if (is_delegated_type && mb.ImportReference(icmn.method_info, t.ElementType).Resolve().IsSpecialName) continue;

                    var mi = mb.ImportReference(icmn.method_info, t.ElementType).AsMemberOf(t);
                ...
                if (icfn != null)
                {
                    var ftype = GetTypeOfGenericInstanceField(t, mb.ImportReference(icfn.compiled_field, t.ElementType));
                    var fi = mb.ImportReference(icfn.compiled_field, t.ElementType).AsMemberOf(t);
        ...
        public override void visit(IForeachNode value)
        {
            ...
            il.Body.Variables.Add(lb);
            if (save_debug_info && false) il.Body.Method.DebugInformation.Scope.Variables.Add( new Mono.Cecil.Cil.VariableDebugInformation(lb, "$enumer$" + uid++) );
