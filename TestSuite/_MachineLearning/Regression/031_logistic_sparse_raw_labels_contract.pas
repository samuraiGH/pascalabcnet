...
begin
  ...
  Check(proba.RowCount = X.RowCount, 'PredictProba row count mismatch');
  Check(proba.ColCount = classes.Length, 'PredictProba class count mismatch');
  ...
  begin
    ...
    Check(labels[i] = pi.ToString, $'Predict and PredictLabels mismatch at {i}');
  end;
end.
